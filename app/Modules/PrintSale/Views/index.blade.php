...
@section('content')
	        ...
	        <div class="col-xs-12">
	          <div class="box box-success">
	            <div class="box-header">
		            ...
		            <div class="box-tools">
		                <a href="{{ url('panel/print-sales/create') }}" class="btn btn-block btn-success">
	            ...
	            <div class="box-body table-responsive">
	              	<table class="table table-striped">
		                ...
		                <tr>
		                  	...
		                  	<th>Price</th>
		                  	<th>Status</th>
		                  	@permission('printsell-update')
		                  		<th>Priority</th>
		                  	@endpermission
		                  	<th>Actions</th>
		                </tr>

		                @if(count($print_sales) > 0)
		                	@foreach($print_sales AS $photo)
		                		<tr>
		                			<td>
				                  	...
				                  	@permission('printsell-update')
					                  	<td>
					                  		@if($photo->status == 1)
						                  		<a href="{{ url('panel/print-sales/'.$photo->id.'/up') }}" class="btn btn-default"><i class="fa fa-arrow-up"></i></a>
						                  		<a href="{{ url('panel/print-sales/'.$photo->id.'/down') }}" class="btn btn-default"><i class="fa fa-arrow-down"></i></a>
					                  		@endIf
					                  	</td>
				                  	@endpermission
				                  	<td>
				                  		<div class="btn-group">
					                      	{{ Form::open(array('url' => 'panel/print-sales/'.$photo->id)) }}
							                    {{ Form::hidden('_method', 'DELETE') }}
							                    <!-- <a href="{{ url('panel/print-sales/'.$photo->id) }}" class="btn btn-default"><i class="fa fa-eye"></i></a> -->
					                      		@permission('printsell-update')
					                      			<a href="{{ url('panel/print-sales/'.$photo->id.'/edit') }}" class="btn btn-default"><i class="fa fa-pencil"></i></a>
					                      		@endpermission
							                    @permission('printsell-delete')
	              	...
	              	<div class="pagination pull-right">
                        {{ $print_sales->appends($_REQUEST)->render() }}
                    </div>
    ...
    <script type="text/javascript">
    	$(document).ready(function () {
        	...
        	page_select('print-sales-manage', 'print-sales', 'Print Sell', 'Manage');
		});
    </script>
